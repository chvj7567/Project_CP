...
- **name**: CatPang
- **one_liner**: Unity로 제작된 Android용 모바일 매치-3 퍼즐 게임 — 고양이 테마, 노멀/하드/보스 3모드, Google Play 출시 목표

## 컨셉 / 단계

- **concept_doc**: `CLAUDE.md`
- **stage**: Beta
- **stage_goal**: 400 스테이지(노멀 150·하드 150·보스 100) 콘텐츠 완성 및 일일 미션·광고·IAP 리텐션 루프 안정화

## 코드 / 인프라

- **engine**: Unity 6 (6000.0.68f1)
- **language**: C#
- **namespace**: 글로벌 (별도 namespace 없음; Data.cs 만 `namespace Data`)
- **architecture**: MVVM (UI 계층 한정)
...
- **stage_json**: `Assets/AssetBundleResources/json/Stage.json` — 스테이지 메타 (group/mode/시간제한/이동횟수 등)
- **stage_block_json**: `Assets/AssetBundleResources/json/StageBlock.json` — 스테이지별 9×9 블록 배치
- **asset_bundle_root**: `Assets/AssetBundleResources/` — Addressables 에셋 루트 (`"Resource"` 라벨)
- **defines_file**: `Assets/Scripts/Defines.cs` — 모든 공용 Enum (EBlockState/EUI/EScene 등)
