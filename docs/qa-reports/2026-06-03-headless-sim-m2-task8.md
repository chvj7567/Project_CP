...
35·73·74·77·82·119. 전부 moveCount=-1(무제한이동) + 점수목표 작음(175~400). RandomAi 가 점수(17000~19000)는 도달하나 **살아있는 Wall(hp>0 6~9개)을 5000턴 안에 다 못 없애** grind. Creator 는 전부 사망(활성=0), 동결 wall 0.

**winnable 검증(`m2-caphitter-greedy.txt`)**: GreedyAi(매치수 최대) 로도 6개 전부 캡(점수 18000~24000, 미클리어). 그러나 이는 sim 버그가 **아님** — Greedy 는 "매치 수"만 최대화해 **벽을 의도적으로 노리지 않는다**. 두 정책 다 벽 인접 매치를 안 만들어 남은 벽 6~9개를 못 깬다. 무제한 이동이므로 **벽-타게팅 정책이면 클리어 가능** → sim 은 정확, 현재 AI 정책의 한계. 이 6개의 클리어율 메트릭을 얻으려면 벽-우선 AI 정책 추가 필요(향후).
