...
## 🤖 개발 방식 — AI 멀티 에이전트 협업 리메이크

> **2023년 직접 1인 개발한 원작**을, **2026.04 ~ 06 AI 멀티 에이전트 협업으로 리메이크**해 Google Play 정식 출시까지 완성했습니다. AI 에게 코드를 "맡긴" 것이 아니라, **사람이 룰과 파이프라인을 설계하고 AI 가 그 안에서 분업**하도록 구성했습니다.

- **역할 분리된 6개 AI 에이전트 파이프라인** — 기획 → 기획 리뷰 → **⛔ 사용자 승인** → 구현 → 코드 리뷰 → 테스트 → 시뮬레이션. 메인 AI 는 조율만 하고 직접 코드를 짜지 않습니다.
  ...
  - `qa-simulator` — 헤드리스 자동 플레이로 스테이지 클리어 가능성·밸런스 검증 리포트
...
- **직접 설계·작성한 공용 인프라 패키지** `com.chvj.unityinfra`(리소스·풀·UI·오디오·광고/IAP/소셜)를 기준 토대로 — AI 코드도 중복 구현 없이 패키지 API 만 호출.
